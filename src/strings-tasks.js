...
function formatTime(minutes, seconds) {
  ...
  const newMin = date.getUTCMinutes().toString().padStart(2, '0');
  const newSec = date.getUTCSeconds().toString().padStart(2, '0');
...
function countVowels(str) {
  ...
  const count = Array.from(str).filter((char) => vowels.has(char)).length;
  return count;
}
...
function extractNameFromTemplate(value) {
  const parts = value.split('Hello, ')[1].split('!')[0];
  return parts.trim();
}
...
function encodeToRot13(str) {
  return str.replace(/[а-яА-Яa-zA-Z]/g, (char) => {
